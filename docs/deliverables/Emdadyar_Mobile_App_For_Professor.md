<!-- rtl-normalized -->

<div dir="rtl" align="right">

# نسخه موبایلی قابل نصب «امداد یار»

## لینک مستقیم

[باز کردن وب‌اپ امداد یار](https://aminhatesprogramming.github.io/emergency-triage-mvp/)

## QR Code

![QR Code نسخه موبایل امداد یار](../artifacts/emdadyar-pwa-qr.png)

## روش نصب روی گوشی اندروید

1. لینک بالا را با Chrome روی گوشی باز کنید.
2. از منوی سه‌نقطه گزینه `Install app` یا `Add to Home screen` را بزنید.
3. آیکن «امداد یار» روی صفحه اصلی گوشی اضافه می‌شود و مثل یک اپلیکیشن اجرا می‌شود.

## نکته مهم اخلاقی و آموزشی

امداد یار یک نمونه آموزشی از سامانه پشتیبان تصمیم‌گیری تریاژ است. خروجی سامانه جایگزین پزشک، پرستار یا پروتکل رسمی درمانی نیست و فقط برای نمایش مسیر محصول، ارزیابی اولیه و تصمیم‌سازی در پروژه درس مدیریت پروژه فناوری اطلاعات ارائه شده است.

## وضعیت نسخه قابل ارسال

| بخش | وضعیت |
|---|---|
| اجرا بدون سرور local | آماده |
| نمایش روی موبایل | آماده |
| نصب به‌صورت PWA | آماده |
| نیاز به حساب کاربری | ندارد |
| نیاز به بک‌اند آنلاین | ندارد؛ مدل سبک در مرورگر اجرا می‌شود |
| مناسب برای جمع‌آوری بازخورد پایلوت | بله |

## سناریوی پیشنهادی برای تست

برای نمایش خروجی، چند داده حیاتی بیمار را وارد کنید و دکمه «ارزیابی بیمار» را بزنید. حتی با چند ورودی محدود نیز سامانه یک ارزیابی اولیه ارائه می‌دهد و میزان کامل بودن اطلاعات را جداگانه نمایش می‌دهد.

</div>

